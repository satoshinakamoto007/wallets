...
def view_current_atomic_swaps(wallet):
    print()
    print(divider)
...
def print_swap_details(swap):
    ...
    print(f"Atomic swap outgoing amount: {swap['amount_outgoing']}")
    print(f"Atomic swap incoming amount: {swap['amount_incoming']}")
...
def view_swaps(wallet):
    if wallet.as_swap_list == []:
        print()
...
def set_amount(wallet, swap_partner):
    print()
    print(f"Your coins: {[x.amount for x in wallet.my_utxos]}")
    print()
    print(f"Enter the amount you want to send to {swap_partner} via your outgoing coin:")
    amount_outgoing = input(prompt)
    try:
        amount_outgoing = int(amount_outgoing)
    except ValueError:
        print()
        print("Invalid input: You entered an invalid amount.")
        return None, None
    if amount_outgoing <= 0:
        print()
        print("Invalid input: The amount must be greater than 0.")
        return None, None
    elif wallet.current_balance <= amount_outgoing:
        print()
        print("Invalid input: This amount exceeds your wallet balance.")
        return None, None
    print()
    print(f"Enter the amount you expect to receive from {swap_partner} via your incoming coin:")
    amount_incoming = input(prompt)
    try:
        amount_incoming = int(amount_incoming)
    except ValueError:
        print()
        print("Invalid input: You entered an invalid amount.")
        return None, None
    if amount_incoming <= 0:
        print()
        print("Invalid input: The amount must be greater than 0.")
        return None, None
    return amount_outgoing, amount_incoming
...
async def set_parameters_init(wallet, ledger_api, as_contacts):
    ...
    while not complete_set_partner:
        ...
        if my_swap_pubkey == None and swap_partner == None and partner_pubkey == None:
            return None, None, None, None, None, None, None, None, "menu"
        else:
            complete_set_partner = True
    complete_set_amount = False
    while not complete_set_amount:
        amount_outgoing, amount_incoming = set_amount(wallet, swap_partner)
        if amount_outgoing == None and amount_incoming == None:
            complete = False
            while not complete:
                print()
                print("Type 'amount' to enter a new set of amounts, or type 'menu' to cancel the atomic swap:")
                choice = input(prompt)
                if choice == "menu":
                    return None, None, None, None, None, None, None, None, "menu"
                elif choice == "amount":
    ...
    return my_swap_pubkey, swap_partner, partner_pubkey, amount_outgoing, amount_incoming, secret, secret_hash, timelock, None
...
async def init_swap_start(wallet, ledger_api, as_contacts):
    print()
    print(divider)
    print(f" {informative} Initiate Atomic Swap {informative}")
    my_swap_pubkey, swap_partner, partner_pubkey, amount_outgoing, amount_incoming, secret, secret_hash, timelock, menu = await set_parameters_init(wallet, ledger_api, as_contacts)
    if menu == "menu":
        print(divider)
    ...
    puzzlehash_outgoing = wallet.as_get_new_puzzlehash(bytes.fromhex(my_swap_pubkey), bytes.fromhex(partner_pubkey), amount_outgoing, timelock_block_outgoing, secret_hash)
    ...
    new_swap = {
    		...
    		"amount_outgoing" : amount_outgoing,
    ...
    wallet.as_swap_list.append(new_swap)
...
async def init_swap_finish(wallet, ledger_api, most_recent_header, as_contacts, puzzlehash_outgoing, tip_index):
    ...
    if puzzlehash_incoming == "menu":
        return
    wallet.as_swap_list[swap_index]["incoming puzzlehash"] = puzzlehash_incoming
    print()
    ...
    print_swap_details(wallet.as_swap_list[swap_index])
    print(divider)
...
def add_secret_hash(wallet):
    ...
    secret_hash = secret_hash_format(secret_hash)
    complete = False
    while not complete:
        try:
...
async def add_puzzlehash(wallet, ledger_api, partner_pubkey, my_swap_pubkey, amount_incoming, timelock, secret_hash, tip_index_at_start, buffer):
    print()
    ...
    try:
        ...
        print("Invalid input: You did not enter a valid puzzlehash.")
        return None, None
    timelock_block_incoming = await receiver_check(wallet, ledger_api, puzzlehash, partner_pubkey, my_swap_pubkey, amount_incoming, timelock, secret_hash, tip_index_at_start, buffer)
...
async def receiver_check(wallet, ledger_api, puzzlehash, partner_pubkey, my_swap_pubkey, amount_incoming, timelock, secret_hash, tip_index_at_start, buffer):
    tip = await ledger_api.get_tip()
    for t in range(tip_index_at_start, tip["tip_index"] + 1):
        timelock_block_incoming = int(timelock + t)
        if puzzlehash_from_string(puzzlehash) == wallet.as_get_new_puzzlehash(bytes.fromhex(partner_pubkey), bytes.fromhex(my_swap_pubkey), amount_incoming, timelock_block_incoming, secret_hash):
...
async def sender_check(wallet, ledger_api, puzzlehash, partner_pubkey, my_swap_pubkey, amount_incoming, timelock, secret_hash, tip_index_at_start):
    tip = await ledger_api.get_tip()
    for t in range(tip_index_at_start, tip["tip_index"] + 1):
        timelock_block_incoming = int(timelock + t)
        if puzzlehash_from_string(puzzlehash) == wallet.as_get_new_puzzlehash(bytes.fromhex(partner_pubkey), bytes.fromhex(my_swap_pubkey), amount_incoming, timelock_block_incoming, secret_hash):
...
async def set_parameters_add(wallet, ledger_api, as_contacts):
    complete_set_partner = False
    while not complete_set_partner:
        my_swap_pubkey, swap_partner, partner_pubkey, tip_index_at_start = await set_partner(wallet, ledger_api, as_contacts, "add")
        if my_swap_pubkey == None and swap_partner == None and partner_pubkey == None:
            return None, None, None, None, None, None, None, None, None, None, None, "menu"
        else:
            complete_set_partner = True
    complete_set_amount = False
    while not complete_set_amount:
    ...
    while not complete_set_puzzlehash:
        puzzlehash, timelock_block_incoming = await add_puzzlehash(wallet, ledger_api, partner_pubkey, my_swap_pubkey, amount_incoming, timelock, secret_hash, tip_index_at_start, buffer)
        if puzzlehash == None:
            complete = False
            while not complete:
                print()
    ...
    return my_swap_pubkey, swap_partner, partner_pubkey, amount_outgoing, amount_incoming, secret, secret_hash, timelock, puzzlehash, timelock_block_incoming, buffer, None


# creates the swap adder's swap
# sets parameters
# creates swap adder's outgoing coin
async def add_swap(wallet, ledger_api, most_recent_header, as_contacts):
    print()
    print(divider)
    print(f" {informative} Add Atomic Swap {informative}")
    my_swap_pubkey, swap_partner, partner_pubkey, amount_outgoing, amount_incoming, secret, secret_hash, timelock, puzzlehash_incoming, timelock_block_incoming, buffer, menu = await set_parameters_add(wallet, ledger_api, as_contacts)
    if menu == "menu":
        print(divider)
        return
    ...
    while not complete_find_coin:
        ...
        for swap in wallet.as_swap_list:
            if puzzlehash == str(swap["outgoing puzzlehash"]):
                return wallet.as_swap_list.index(swap), "outgoing"
            elif puzzlehash == str(swap["incoming puzzlehash"]):
                return wallet.as_swap_list.index(swap), "incoming"
        print()
...
def update_secret(wallet, swap_index):
    swap = wallet.as_swap_list[swap_index]
    complete = False
    while not complete:
        print()
...
def spend_with_secret(wallet, swap_index):
    swap = wallet.as_swap_list[swap_index]
    if swap["secret"] == "unknown":
        menu = update_secret(wallet.as_swap_list, swap_index)
    ...
    while not complete_spend_with_secret:
        ...
        if response == "y":
            ...
            while not complete:
                ...
                response = input(prompt)
                if response == "y":
                    update_secret(wallet.as_swap_list, swap_index)
                    complete = True
                elif response == "n":
                    print()
                    print("A secret is required to spend this atomic swap coin. Please try again when you know the secret.")
    ...
    spend_bundle = wallet.as_create_spend_bundle(swap["incoming puzzlehash"], swap["amount_incoming"], int(swap["timelock block height incoming"]), secret_hash, as_pubkey_sender = bytes.fromhex(swap["partner pubkey"]), as_pubkey_receiver = bytes.fromhex(swap["my swap pubkey"]), who = "receiver", as_sec_to_try = swap["secret"])
    return spend_bundle


# spends a pending atomic swap coin using the coin's timelock condition (used for outgoing coins)
def spend_with_timelock(wallet, swap_index):
    swap = wallet.as_swap_list[swap_index]
    spend_bundle = wallet.as_create_spend_bundle(swap["outgoing puzzlehash"], swap["amount_outgoing"], int(swap["timelock block height outgoing"]), swap["secret hash"], as_pubkey_sender = bytes.fromhex(swap["my swap pubkey"]), as_pubkey_receiver = bytes.fromhex(swap["partner pubkey"]), who = "sender", as_sec_to_try = swap["secret"])
    return spend_bundle


# removes atomic swap coin info from as_contacts and throughout the wallet after the swap's pending coins are spent
...
def spend_coin(wallet, as_contacts):
    ...
    swap_index, whichpuz = find_coin(wallet)
    if swap_index == None and whichpuz == None:
        print(divider)
        return
    swap = wallet.as_swap_list[swap_index]
    if whichpuz == "incoming":
        spend_bundle = spend_with_secret(wallet, swap_index)
    elif whichpuz == "outgoing":
        spend_bundle = spend_with_timelock(wallet, swap_index)
    print(divider)
    return spend_bundle
...
async def get_update(wallet, ledger_api, most_recent_header, as_contacts):
    ...
    for body in update_list:
        ...
        remove_swap_instances(wallet, as_contacts, removals)
        wallet.notify(additions, removals)


async def update_ledger(wallet, ledger_api, most_recent_header, as_contacts):
    print()
    print(divider)
...
async def farm_block(wallet, ledger_api, as_contacts):
    print()
    print(divider)
...
async def main_loop():
    ...
    while selection != "q":
        ...
        if selection == "2":
            ...
            view_current_atomic_swaps(wallet)
        elif selection == "7":
            spend_bundle, puzzlehash_outgoing, tip = await init_swap_start(wallet, ledger_api, as_contacts)
            if spend_bundle is not None:
                await ledger_api.push_tx(tx=spend_bundle)
                await farm_block(wallet, ledger_api, as_contacts)
                await init_swap_finish(wallet, ledger_api, most_recent_header, as_contacts, puzzlehash_outgoing, tip["tip_index"])
        elif selection == "8":
            spend_bundle = await add_swap(wallet, ledger_api, most_recent_header, as_contacts)
            if spend_bundle is not None:
                await ledger_api.push_tx(tx=spend_bundle)
                await farm_block(wallet, ledger_api, as_contacts)
        elif selection == "9":
            spend_bundle = spend_coin(wallet, as_contacts)
            if spend_bundle is not None:
                await ledger_api.push_tx(tx=spend_bundle)
        elif selection == "10":
            await update_ledger(wallet, ledger_api, most_recent_header, as_contacts)
        elif selection == "11":
            most_recent_header = await farm_block(wallet, ledger_api, as_contacts)
