...
class AS_Runnable(Runnable):
    ...
    def view_contacts_details(self, as_contacts):
        print()
        print(divider)
        print(" \u2447 View Contacts \u2447")
        choice = "view"
        while choice == "view":
            self.view_contacts(as_contacts)
            if as_contacts == {}:
                print(divider)
                return
            choice = "invalid"
    ...
    def edit_contact(self, wallet, as_contacts):
        print()
        print(divider)
        print(" \u2447 Edit Contact \u2447")
        choice = "edit"
    ...
    def view_current_atomic_swaps(self, as_swap_list):
        print()
        print(divider)
        print(" \u2447 View Current Atomic Swaps \u2447")
        self.view_swaps(as_swap_list)
        print(divider)
    ...
    def set_partner(self, wallet, as_contacts):
        self.view_contacts(as_contacts)
        if as_contacts == {}:
            print()
    ...
    def set_parameters_init(self, wallet, as_contacts):
        choice = "partner"
        menu = None
        while choice == "partner":
            swap_partner = self.set_partner(wallet, as_contacts)
        ...
        choice = "amount"
        while choice == "amount":
            amount = self.set_amount(wallet, as_contacts, "init")
            if amount == None:
                choice = "invalid"
                while choice == "invalid":
                    print()
        ...
        choice = "time"
        while choice == "time":
            timelock = self.set_timelock(wallet, as_contacts)
            if timelock == None:
                choice = "invalid"
                while choice == "invalid":
                    print()
    ...
    def init_swap(self, wallet, as_contacts, as_swap_list, my_pubkey_orig):
        print()
        print(divider)
        ...
        puzzlehash_incoming = self.add_puzzlehash_init(wallet)
    ...
    def set_parameters_add(self, wallet, as_contacts, as_swap_list):
        choice = "partner"
        menu = None
        while choice == "partner":
            swap_partner = self.set_partner(wallet, as_contacts)
        ...
        choice = "amount"
        while choice == "amount":
            amount = self.set_amount(wallet, as_contacts, "add")
            if amount == None:
                choice = "invalid"
                while choice == "invalid":
                    print()
        ...
        while choice == "time":
            ...
            if timelock == None:
                ...
                choice = "continue"
        secret = self.set_secret("add")
        choice = "hash"
        while choice == "hash":
            secret_hash = self.add_secret_hash(wallet)
            if secret_hash == None:
                choice = "invalid"
                while choice == "invalid":
                    print()
        ...
        while choice == "puzzlehash":
            puzzlehash = self.add_puzzlehash(wallet)
            if puzzlehash == None:
                choice = "invalid"
                while choice == "invalid":
                    print()
    ...
    def find_coin(self, as_swap_list):
        choice = "find"
        while choice == "find":
            self.view_swaps(as_swap_list)
            print()
    ...
    def spend_with_secret(self, wallet, as_swap_list, swap_index, my_pubkey_orig):
        swap = as_swap_list[swap_index]
        if swap["secret"] == "unknown":
            menu = self.update_secret(as_swap_list, swap_index)
        ...
        while choice == "secret":
            ...
            if response == "y":
                ...
                while choice == "invalid":
                    ...
                    response = input(prompt)
                    if response == "y":
                        self.update_secret(as_swap_list, swap_index)
                        choice = "secret"
                    elif response == "n":
                        print()
                        print("A secret is required to spend this atomic swap coin. Please try again when you know the secret.")
    ...
    def spend_coin(self, wallet, as_contacts, as_swap_list, my_pubkey_orig):
        print()
        print(divider)
        ...
        if whichpuz == "incoming":
            spend_bundle = self.spend_with_secret(wallet, as_swap_list, swap_index, my_pubkey_orig)
        elif whichpuz == "outgoing":
            spend_bundle = self.spend_with_timelock(wallet, as_swap_list, swap_index, my_pubkey_orig)
        print(divider)
        return spend_bundle

    async def update_ledger(self, wallet, ledger_api, most_recent_header, as_contacts, as_swap_list):
        print()
        print(divider)
        print(" \u2447 Get Update \u2447")
        ...
        for body in update_list:
            ...
            self.remove_swap_instances(wallet, as_contacts, as_swap_list, removals)
            wallet.notify(additions, removals, as_swap_list)
        print()
        print("Update complete.")
        print(divider)

    async def farm_block(self, wallet, ledger_api, as_contacts, as_swap_list):
        print()
        print(divider)
        print(" \u2447 Commit Block \u2447")
        print()
    ...
    async def main(self):
        ...
        while selection != "q":
            ...
            if selection == "2":
                self.view_funds(wallet, as_swap_list)
            elif selection == "3":
                self.view_contacts_details(as_contacts)
            elif selection == "4":
                self.add_contact(wallet, as_contacts)
            elif selection == "5":
                self.edit_contact(wallet, as_contacts)
            elif selection == "6":
                self.view_current_atomic_swaps(as_swap_list)
            elif selection == "7":
                spend_bundle = self.init_swap(wallet, as_contacts, as_swap_list, my_pubkey_orig)
                if spend_bundle is not None:
                    await ledger_api.push_tx(tx=spend_bundle)
            elif selection == "8":
                spend_bundle = self.add_swap(wallet, as_contacts, as_swap_list, my_pubkey_orig)
                if spend_bundle is not None:
                    await ledger_api.push_tx(tx=spend_bundle)
            elif selection == "9":
                spend_bundle = self.spend_coin(wallet, as_contacts, as_swap_list, my_pubkey_orig)
                if spend_bundle is not None:
                    await ledger_api.push_tx(tx=spend_bundle)
            elif selection == "10":
                await self.update_ledger(wallet, ledger_api, most_recent_header, as_contacts, as_swap_list)
            elif selection == "11":
                most_recent_header = await self.farm_block(wallet, ledger_api, as_contacts, as_swap_list)
